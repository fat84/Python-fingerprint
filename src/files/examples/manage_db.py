...
def update_work_clock_in(user):
    ...
    query = ("UPDATE work_log set status = '" + status + "', clock_in_time ='" + str(nowTime) + "', clock_in_sec = " + str(nowSec) + " where user_id = " + userId + " and work_date = " + getTodayDate())
    print(query)
    if(is_connected()):
        conn = mysql.connector.connect(host=host,
                                       database='attendance',
...
def clock_in_to_text(sectionlogId,clock_in_sec,finger):
    now = datetime.now()
    ...
    if (nowSec > int(clock_in_sec) + 900):
        status = "late"
    strfinger = str(finger).replace("[", "")
    strfinger = str(strfinger).replace("]", "")
    query = ("UPDATE `section_log` set status = '" + status + "', finger = '" + strfinger + "', clock_in_sec = '" + str(
...
def clock_in(sectionlogId,clock_in_sec,finger):
    global  conn
    try:
        conn = mysql.connector.connect(host=host,
                                       database='attendance',
        ...
        strfinger = str(finger).replace("[", "")
        strfinger = str(strfinger).replace("]", "")
        query = ("UPDATE `section_log` set status = '" + status + "', finger = '" + strfinger + "', clock_in_sec = '" + str(nowSec) + "' where id = " + str(sectionlogId))
        cursor.execute(query)
        conn.commit()
        conn.close()
